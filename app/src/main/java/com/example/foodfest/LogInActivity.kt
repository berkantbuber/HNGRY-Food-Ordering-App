...
import com.example.foodfest.databinding.ActivityLogInBinding
            ...
            if (email.isBlank() || password.isBlank()) {
                ...
                auth.signInWithEmailAndPassword(email, password).addOnCompleteListener { task ->
                    if (task.isSuccessful) {
                        ...
                        updateUi(user)
                        Toast.makeText(this, "Oturum açma başarılı", Toast.LENGTH_SHORT)
                            .show()
                    } else {
                        auth.createUserWithEmailAndPassword(email, password)
                            .addOnCompleteListener { tasek ->
                                if (tasek.isSuccessful) {
                                    ...
                                    Toast.makeText(
                                        this,
                                        "Hesap oluşturulamadı : ${tasek.exception}",
                                        Toast.LENGTH_SHORT
                                    ).show()
                                }
                            }
                ...
                if (task.isSuccessful) {
                    ...
                    auth.signInWithCredential(credential).addOnCompleteListener { tasek ->
                        if (tasek.isSuccessful) {
                            ...
                            Toast.makeText(
                                this,
                                "Google ile başarılı şekilde oturum açıldı.",
                                Toast.LENGTH_SHORT
                            ).show()
                        } else {
                            Toast.makeText(
